...
class Dataset:
    ...
    def load_multiclass_dataset(self, path, height=224, width=224, num_channels=3):
        ...
        # images = [sorted(os.listdir(os.path.join(path, id))) for id in classes]
        images = []
        for id in classes:
            images += [sorted(img for img in os.listdir(os.path.join(path, id)) if img.endswith('.png'))]
    ...
    def load_images(self, path, height=224, width=224, num_channels=3):
        ...
        for j in range(len(images)):
                ...
                img = Image.open(os.path.join(path, images[j]))
                img = np.asarray(img).reshape(height, width, num_channels)
